(function() {
  ...
  function init() {
	...
	window.addEventListener('keypress', function(e) { if (e.key === "Enter") { login(); } });
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        onLoadPositionFailed, {
          maximumAge: 60000 // acquire geo-location in cache last 60 sec.
        });
...
//            showLoadingMessage('Redirecting to login page... , if NOT click "Back to Login" button');
        } else {
    ...
    ajax(method, url, req,
      ...
      function(res) {
        ...
        if (result.status === 'OK' || result.result === 'SUCCESS') {
          ...
          if(btn.className === 'main-nav-btn active') {
//        	  loadFavoriteItems();
        	  var ui = document.querySelector('#item-list');
        	  ui.removeChild(li);
          }
        }
      });
